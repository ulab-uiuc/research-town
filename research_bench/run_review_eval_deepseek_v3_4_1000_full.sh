...
for MODE in "${MODES[@]}"
do
    # OUTPUT_PATH="${OUTPUT_DIR}/iclrbench_result_deepseek_v3_${MODE}_200_full.jsonl" # top_k 1
    OUTPUT_PATH="${OUTPUT_DIR}/iclrbench_result_deepseek_v3_${MODE}_200_full_topk_5.jsonl"
    echo "Running evaluation for mode: $MODE"
    CUDA_VISIBLE_DEVICES=9 poetry run python run_review_eval.py --input "$INPUT_PATH" --output "$OUTPUT_PATH" --mode "$MODE" --num_processes "$NUM_PROCESSES" --config_path ../configs_deepseek_v3_1024
