...
def download_papers(
    keywords: str, start_date: str, end_date: str, save_dir: str
) -> None:
    """
        ...
        if 'agent' in paper.title.lower():
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f'Failed to download: {paper.title} (Status Code: {response.status_code})'
                    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Download AI-related papers from arXiv based on search criteria.'
    )
