...
OUTPUT_DIR="./results"
MODES=("author_citation" ) #  "citation_only" "zero_shot" "author_only"
...
run_eval() {
    MODE=$1
    OUTPUT_PATH="${OUTPUT_DIR}/paperbench_result_deepseek_v3_${MODE}_1000_full_4096.jsonl"
    echo "Running evaluation for mode: $MODE"
    poetry run python run_proposal_eval.py --input "$INPUT_PATH" --output "$OUTPUT_PATH" --mode "$MODE" --num_processes 1 --config_path "../configs_deepseek_v3"
