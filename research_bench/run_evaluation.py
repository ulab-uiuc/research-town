...
def get_current_5q(intro: str) -> Optional[str]:
    ...
    try:
        ...
        if response and len(response) > 0 and len(response[0]) > 0:
            ...
            logger.warning("Received empty response from model_prompting for current_5q.")
            return None
...
def get_proposal_5q(authors: List[str], intros: List[str]) -> Optional[str]:
    ...
    try:
        ...
        if not leader_profile:
            ...
            return None
        logger.info(f"Created leader agent for profile")
...
def compute_gpt_metric(current_5q: str, proposal_5q: str) -> Optional[float]:
    ...
    try:
        ...
        if response and len(response) > 0 and len(response[0]) > 0:
            ...
            logger.warning("Received empty response from model_prompting for GPT metric.")
            return None
    except Exception as e:
...
def process_paper(paper_key: str, paper_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
        ...
        for ref in references:
            ...
            if arxiv_ref_id:
                ...
                if ref_intro:
                    ...
                    logger.warning(f"Introduction not found for referenced paper: {ref_url}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Automated Evaluation of Tool-Augmented LLMs as Conversational AI Agents")
