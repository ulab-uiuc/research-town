...
run_eval() {
    local MODE=$1
    OUTPUT_PATH="${OUTPUT_DIR}/paperbench_result_qwen_${MODE}_1000_full_4096.jsonl"
    echo "Running evaluation for mode: $MODE"
    poetry run python run_proposal_eval.py --input "$INPUT_PATH" --output "$OUTPUT_PATH" --mode "$MODE" --num_processes 1 --config_path "../configs_qwen"
