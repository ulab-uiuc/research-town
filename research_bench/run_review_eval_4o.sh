...
for MODE in "${MODES[@]}"
do
    OUTPUT_PATH="${OUTPUT_DIR}/oodbench_result_4o_${MODE}_topk_3.jsonl"
    echo "Running evaluation for mode: $MODE"
    poetry run python run_review_eval.py --input "$INPUT_PATH" --output "$OUTPUT_PATH" --mode "$MODE" --num_processes "$NUM_PROCESSES" --config_path ../configs_4o
