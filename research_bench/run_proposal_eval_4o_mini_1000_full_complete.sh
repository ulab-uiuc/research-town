...
run_eval() {
    MODE=$1
    OUTPUT_PATH="${OUTPUT_DIR}/paper_bench_merged_result_4o_mini_${MODE}_resplit_delete_incomplete.jsonl"
    echo "Running evaluation for mode: $MODE"
    poetry run python run_proposal_eval.py --input "$INPUT_PATH" --output "$OUTPUT_PATH" --mode "$MODE" --num_processes 1 --config_path "../configs_8192"
