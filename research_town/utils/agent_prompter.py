...
def generate_profile(
    ...
    template_input = {"personalinfo": info}
...
def generate_review_score(
    ...
    prompt_qa = (
        ...
        "Here are the reviews: {review_content}"
    )
...
def generate_idea(
    ...
    """
    prompt_template = (
        "Here is a high-level summarized trend of a research field {research_trend}. "
    ...
    template_input = {"research_trend": research_trend}
    prompt = prompt_template.format_map(template_input)
    return openai_prompting(llm_model, prompt)
...
def summarize_research_field_prompting(
    ...
    query_template = (
        ...
        "Here is my profile: {agent_profile}"
        "Here are the keywords: {research_keywords}"
    )
    template_input = {"agent_profile": agent_profile, "research_keywords": research_keywords}
    query = query_template.format_map(template_input)

    corpus = []
    for timestamp in papers.keys():
        for paper in papers[timestamp]:
...
def find_collaborators_prompting(
    ...
    self_serialize = [
        f"Name: {name}\nProfile: {agent_self_profile[name]}" 
        for _, name in enumerate(agent_self_profile.keys())
    ]
    self_serialize_all = "\n\n".join(self_serialize)
    ...
    collaborator_serialize = [
        f"Name: {name}\nProfile: {agent_collaborator_profiles[name]}" for _, name in enumerate(agent_collaborator_profiles.keys())]
...
def write_paper_abstract_prompting(
    ...
    """
    ideas_serialize = [f"{i}: {idea}" for i, idea in enumerate(research_ideas)]
    ideas_serialize_all = "\n".join(ideas_serialize)

    papers_serialize = []

    for i, timestamp in enumerate(papers.keys()):
        abstracts = [paper['abstract'] for paper in papers[timestamp]]
    ...
    template_input = {
        "ideas_serialize_all": ideas_serialize_all,
...
def review_paper_prompting(
    ...
    papers_serialize = []
    for _, timestamp in enumerate(paper.keys()):
...
def make_review_decision_prompting(
    ...
    review_serialize = []
    for _, name in enumerate(paper_review.keys()):
        content = paper_review[name]
    ...
    template_input = {
        "submission_serialize_all": submission_serialize_all,
...
def rebut_review_prompting(
    ...
    decision_serialize = []
    for _, name in enumerate(paper_decision.keys()):
        content = paper_decision[name]
