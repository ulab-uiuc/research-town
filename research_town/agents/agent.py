...
class Agent(object):
    ...
    def write_review(
        ...
        paper_text: Optional[PaperText] = None,
    ) -> Review:
        if not (proposal or paper_text):
            raise ValueError('Either proposal or paper_text must be provided')

        # Serialize input and select appropriate prompting function and templates
        if proposal:
