...
class PaperSubmissionMultiAgentEnvironment(BaseMultiAgentEnv):
    ...
    def step(self) -> None:
        ...
        agent_names_to_objs = {self.agents[iter_agent].name: self.agents[iter_agent] for iter_agent in self.agents} # map from real names to agent objects
        abstracts = {}
        for agent_name, agent in self.agents.items():
            ...
            for researcher_name in collaborators:
                if researcher_name not in agent_names_to_objs:
                    ...
                    collaborator_agents.append(agent_names_to_objs[researcher_name])
            ideas = agent.generate_idea(trends=trends, domain="machine learning")
            for collaborator_agent in collaborator_agents:
                ideas.extend(collaborator_agent.generate_idea(trends=trends, domain="machine learning"))
