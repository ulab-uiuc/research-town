...
class BaseEngine:
    def __init__(
        ...
        self.agent_db, self.paper_db, self.progress_db, self.env_db = (
            agent_db,
            paper_db,
            progress_db,
            env_db,
    ...
    def start(self, task: str, env_name: str = 'start') -> None:
        ...
        self.curr_env.on_enter(
            self.time_step, self.stop_flag, [proj_leader], ['proj_leader'], ['gpt-4o']
        )

    def transition(self) -> None:
        pass_or_fail = self.curr_env.on_exit()
        next_env_name = self.transitions[self.curr_env_name][pass_or_fail]
        input_data = self.transition_funcs.get(
            (self.curr_env_name, next_env_name), lambda x: {}
        )(self.curr_env)
