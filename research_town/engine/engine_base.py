...
class BaseResearchEngine:
    ...
    def find_proj_participants(self,potential_participants_profiles:List[AgentProfile]) -> List[str]:
        leader_profile=self.leader.bio
        self.proj_participants_pks=self.agent_db.match(query=leader_profile,agent_profiles=potential_participants_profiles)
        return self.proj_participants_pks

    def find_proj_leaders(self, leader_profile:AgentProfile) -> None:
        self.leader=leader_profile

    def find_reviewers(self,paper_submission:ResearchPaperSubmission,potential_reviewers_profiles:List[AgentProfile]) -> List[str]:
        paper_abstract=paper_submission.abstract
        self.reviewers_pks = self.agent_db.match(query=paper_abstract,
                                                    agent_profiles=potential_reviewers_profiles)
    ...
    def read_papers(self,potential_paper_profiles:List[PaperProfile])-> List[str]:
        leader_profile = self.leader.bio
        paper_dks=self.paper_db.match(query=leader_profile,paper_profiles=potential_paper_profiles)
        return paper_dks
