...
from research_town.evaluators import ReviewEvalOutput, ReviewQualityEvaluator
...
class review_content_eval_db(object):
    def __init__(self) -> None:
        ...
        self.selected_paper_reviews: Dict[
            str, ReviewContentEval
        ] = {}  # save the paper reviews to be evaluated
        ...
        with open(file_name, 'r') as f:
            ...
            if paper_type == 'openreview':
                ...
                for title, details in raw_data_papers.items():
                    details['contents'] = details.pop(
                        'sim_contents', []
                    )  # set 'contents' key from 'sim_contents'
            else:
    ...
    def select_paper_reviews(self, paperreview_num: int) -> List[ReviewContentEval]:
        ...
        for paperreview in self.data.values():
            ...
            count += 1
            self.selected_paper_reviews[select.title] = (
    ...
    def eval_research_content(
        ...
        assert len(review.contents) > 0, 'Error: no reviews\n'
        ...
        self.selected_paper_reviews[
            review.title
        ].overall_score = review_quality.overall_score
        self.selected_paper_reviews[
            review.title
        ].dimension_scores = review_quality.dimension_scores
...
def main(
    ...
    for idx, paper_review in enumerate(
        ...
        eval = review_content_eval.eval_research_content(
            review=paper_review, model_name=model_name
        )
