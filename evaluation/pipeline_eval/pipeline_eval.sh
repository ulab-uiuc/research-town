...
for domain_option in "${domain_options[@]}"; do
    for evaluator_model_option in "${evaluator_model_options[@]}"; do
        for agent_model_option in "${agent_model_options[@]}"; do
            # run
            python evaluation/pipeline_eval/eval_pipeline.py --eval_log_num="$eval_log_num" --domain="$domain_option" --agent_model_name="$agent_model_option" --evaluator_model_name="$evaluator_model_option"
        done
    done
done
