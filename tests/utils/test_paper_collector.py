...
def test_get_related_papers() -> None:
    with (
        ...
        patch(
            'research_town.utils.paper_collector.neiborhood_search'
        ) as mock_neiborhood_search,
    ):
        ...
        mock_neiborhood_search.return_value = torch.tensor([[0, 1]])
...
def test_neiborhood_search() -> None:
    ...
    num = 1
    result = neiborhood_search(query_data, corpus_data, num)
    assert result == [[0]]
