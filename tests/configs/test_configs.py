...
def test_default_initialization() -> None:
    ...
    assert config.param.participant_num == 3
...
def test_yaml_loading() -> None:
    ...
    assert config.param.participant_num == 2
...
def test_merging_configurations() -> None:
    base_config = {
        'param': {'related_paper_num': 10, 'participant_num': 10},
    ...
    assert config.param.related_paper_num == 10
    assert config.param.participant_num == 10
    ...
    for yaml_file in os.listdir(tmpfile_path):
        if yaml_file.endswith('.yaml'):
            ...
            if 'config' in yaml_file:
                ...
                assert loaded_data['participant_num'] == 3
