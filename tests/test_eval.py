...
def test_eval_GeneralQuality_idea(use_mock:bool):
    ...
    if use_mock:
        with patch("research_town.utils.eval_prompter.GeneralQuality_idea_EvalPrompting.model_prompting", MagicMock(return_value=[
...
def test_eval_GeneralQuality_paper(use_mock:bool):
    ...
    if use_mock:
        with patch("research_town.utils.eval_prompter.GeneralQuality_paper_EvalPrompting.model_prompting", MagicMock(return_value=[
            "Overall Score=86. Dimension Scores=[9, 8, 9, 9, 8, 8, 8, 9, 8, 8]."
        ])):
